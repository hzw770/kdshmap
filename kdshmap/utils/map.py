...
###### NEW ######
def filter_weight_for_state(prop_array, density0, t_list, noise_op, trunc_freq=None, prop_superop_array_fft=None):
    """
    Given a qutip.propagator (converted to a numpy array) and a noise operator,
    this function returns the frequencies, the filter strength at different frequencies,
    and the filter operators (superop) at different frequencies. Different from the previous function, the current
    takes into account the initial state of the system, therefore ignore irrelevant dissipation channels between
    unpopulated system states.
    ...
    lindb = np.einsum('ijklm->ijmkl', lindb)
    lindb = lindb.reshape(len(fk_list), dimension*dimension, dimension*dimension)
    ...
    return fk_list, np.sqrt(filter_weights_state.real), filter_op
