...
def generate_filter(H: Union[list, q.qobj.Qobj],
    ...
    fk_list, filter_strength, _ = filter_weight(prop_array, t_list, noise_op, trunc_freq)
    return fk_list, filter_strength


def plot_filter(H, t_list, noise_op, trunc_freq=(0, 0), options=q.Options(atol=1e-10, rtol=1e-10), solver='qutip', u0_list=None,
                filters=None, ax=None):

    if filters is None:
        filters = [None, None]

    if any(x is None for x in filters):
        fk_list, filter_strength = generate_filter(H, t_list, noise_op, trunc_freq=trunc_freq, options=options, solver='qutip', u0_list=u0_list)
    else:
...
def plot_filter_Sf(H: Union[list, q.qobj.Qobj],
    ...
    if (trunc_freq[1]-trunc_freq[0]) == 0:
...
def plot_filter_Sf_multiple(H: Union[list, q.qobj.Qobj],
    ...
    if len(noise_ops) == 1:
        ...
        return plot_filter_Sf(H, t_list, noise_ops[0], f_list_list[0], Sf_list_list[0], trunc_freq=trunc_freq_list[0],
                              options=options, solver=solver, u0_list=u0_list, filters=filters_list[0], ax=None)
