...
from ..utils.map import filter_weight_for_state
import qutip as q
import numpy as np
...
def generate_filter_for_state(H: Union[list, q.qobj.Qobj],
    ...
    fk_list, filter_weights_state, _ = filter_weight_for_state(prop_array, density0, t_list, noise_op, trunc_freq, prop_superop_array_fft=prop_superop_array_fft)
...
def plot_filter_for_state(H, density0, t_list, noise_op, trunc_freq=None, options=dict(atol=1e-10, rtol=1e-10), solver_type='qutip', u0_list=None,
                filters=None, ax=None, prop_array: np.ndarray = None, prop_superop_array_fft: np.ndarray = None):
    ...
    fk_list, filter_weights_state = generate_filter_for_state(H, density0, t_list, noise_op, trunc_freq=trunc_freq, options=options,
                                                solver_type='qutip', u0_list=u0_list, prop_array=prop_array,
                                                prop_superop_array_fft=prop_superop_array_fft)
...
def plot_filter_Sf_for_state(H: Union[list, q.qobj.Qobj],
                   density0: q.qobj.Qobj,
                   t_list: np.ndarray,
                   noise_op: q.qobj.Qobj,
                   f_list: np.ndarray,
    ...
    plot_filter_for_state(H, density0, t_list, noise_op, trunc_freq=trunc_freq, options=options, solver_type=solver_type, u0_list=u0_list,
                filters=filters, ax=ax, prop_array=prop_array, prop_superop_array_fft=prop_superop_array_fft)
    ax2 = ax.twinx()
    ax2.plot(f_list, Sf_list, lw=2, alpha=1, color='k')
    ax2.set_xlim(trunc_freq[0], trunc_freq[1])
...
def plot_filter_Sf_for_state_multiple(H: Union[list, q.qobj.Qobj],
                            density0: q.qobj.Qobj,
                            t_list: np.ndarray,
                            noise_ops: list,
                            f_list_list: list,
    ...
    if len(noise_ops) == 1:
        ...
        return plot_filter_Sf_for_state(H, density0, t_list, noise_ops[0], f_list_list[0], Sf_list_list[0], trunc_freq=trunc_freq_list[0],
                              options=options, solver_type=solver_type, u0_list=u0_list, filters=None, ax=None,
                              prop_array=prop_array, prop_superop_array_fft=prop_superop_array_fft)
    ...
    for n_ in range(len(noise_ops)):
        ...
        plot_filter_Sf_for_state(H, density0, t_list, noise_ops[n_], f_list_list[n_], Sf_list_list[n_], trunc_freq=trunc_freq,
                       options=options, solver_type=solver_type, u0_list=u0_list, filters=None,
                       ax=ax[n_], prop_array=prop_array, prop_superop_array_fft=prop_superop_array_fft)
    plt.tight_layout()
    return ax